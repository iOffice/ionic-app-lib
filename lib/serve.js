...
Serve.startServer = function startServer(options, app) {
  ...
  var server = http.createServer(Server.middleware);
...
Serve.middleware = function middleware(req, res, next, options) {
...
Serve.start = function start(options) {
  ...
  try {
    ...
    // })
    .then(function(data) {
      return Serve.browserSyncServer(options);
    })
    // .then(function() {
...
Serve.browserSyncServer = function browserSyncServer(options) {
  var syncOptions = {};

  if (!options.launchBrowser) {
    syncOptions.open = false;
  }

  console.log('options', options);

  syncOptions.files = options.watchPatterns;

  syncOptions.port = options.port;

  syncOptions.server = {baseDir: path.join(options.appDirectory, 'www'), index: 'index.html'};

  syncOptions.server.middleware = function(req, res, next) {
    Serve.middleware(req, res, next, options); 
    // next();
  }
