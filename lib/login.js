...
Login.requestLogIn = function requestLogin(email, password, saveCookies) {
  ...
  request({
    ...
    try {
      // we're going to save the users email and account ID to the ionic.config file.
      var jsonResponse = JSON.parse(response.body);
      console.log('json Response:', jsonResponse);
      var ionicConfig = new IonicStore('ionic.config');
      ionicConfig.set('email', email.toString());
      ionicConfig.set('id', jsonResponse.id);
      ionicConfig.save();
    } catch (ex) {
      logging.logger.error('Ex:', ex);
    }
    // Login.saveUserInformation(email.toString(), )
...
Login.getUserApps = function getUserApps(jar) {
  var q = Q.defer();
  var url = [settings.IONIC_DASH, settings.IONIC_API, 'apps?format=json'].join('');
...
Login.getDownloadLink = function getDownloadLink(jar, appId) {
  var q = Q.defer();
  var url = [settings.IONIC_DASH, settings.IONIC_API, 'app/', appId, '/updates/download'].join('');
